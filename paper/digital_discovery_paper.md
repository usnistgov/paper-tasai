...
Here we argue that no single controller should be asked to solve all stages simultaneously. Instead, we introduce **TAS-AI** (**Triple-Axis Spectrometer Artificial Intelligence**), a hybrid workflow that begins with agnostic discovery and then hands control to a physics-informed, motion-aware inference engine once enough structure exists to justify model-based planning. This framing turns an apparent weakness into a design principle: if agnostic methods outperform physics-informed planning on blind global reconstruction, that is evidence that discovery and inference are distinct problems rather than evidence against model-based autonomy.
TAS-AI also supports structure-informed Hamiltonian proposal generation from crystal structure using exchange-path analysis and Goodenough-Kanamori-Anderson heuristics, followed by data-driven discrimination among candidate models within the TAS-AI workflow.
...
**Scope of this paper.** To make the boundaries of the present work explicit at the outset: the four-layer controller of Figure 1—agnostic discovery, physics-informed discrimination/refinement, motion-aware execution, and the strategic audit layer—is **implemented and benchmarked** in this manuscript. All quantitative claims in Sections 4 and 5, including the blind-reconstruction benchmarks, the controlled refinement and motion studies, the in-loop Hamiltonian discrimination test, the audit-layer ablations, and the NiPS3 real-material demonstration of Section 6.1, are produced by this controller running against a high-fidelity digital twin. The strategic audit layer is **implemented and pilot-demonstrated** in Section 5: the 90-measurement closed-loop run of Figure 10 and the ghost-optic, bilayer, and multi-model-trap ablations of Section 5.3 exercise it under the same kinematic and counting constraints as the non-LLM loop, on synthetic data rather than against a live spectrometer. Outlook items — including live-instrument deployment, broader Hamiltonian families, and fully autonomous candidate-library generation from crystal structure — are addressed in §6 and are explicitly not claimed as results here.

The manuscript is organized around those tasks. Section 2 introduces the hybrid workflow and formal problem statement. Section 3 describes the agnostic discovery policy, the physics-informed inference engine, the instrument model, and batch planning. Section 4 presents the benchmark results in the order the workflow is actually used: blind discovery, physics-informed inference, wall-clock optimization, and a fully integrated hybrid handoff. Section 5 addresses the algorithmic myopia failure mode and presents the constrained audit layer, including its design, pilot closed-loop results, and targeted ablations. Section 6 discusses implemented structure-informed candidate generation, a real-material demonstration, and hardware deployment. Section 7 concludes.
...
1. **Agnostic discovery.** A Log-GP mapper performs initial coverage of accessible $(Q,E)$ space and proposes additional measurements to localize signal support without assuming a Hamiltonian.
2. **Physics-informed discrimination and refinement.** Once signal structure is present, a Hamiltonian-aware planner ranks candidate measurements by expected information gain and model contrast per unit time, using a candidate-model family that may be hand-specified or seeded from structure-informed proposals.
3. **Motion-aware execution.** Accepted probes are ordered to reduce wall-clock overhead, either greedily or with a short-horizon MCTS batch planner when path dependence matters.
4. **Strategic audit layer (implemented; optional at runtime).** When posterior-weighted planning becomes myopic, a constrained router may request a small number of falsification probes before normal planning resumes. The layer is fully implemented and pilot-demonstrated in §5; it is "optional" only in the sense that it can be disabled when the posterior-weighted planner is performing well.

This decomposition is deliberate. It allows the experiment to use the right inductive bias at the right time: broad exploration when little is known, then model-based discrimination and refinement once the physics becomes informative. In the current framework, the candidate-model family in Layer 2 may be supplied directly by the user or seeded from crystal structure through exchange-path analysis and Goodenough-Kanamori-Anderson heuristics. Executed measurements then update the experiment state and return the controller to the next cycle.

![Figure 1](figures/closed_loop_workflow.png){ width=6in }

**Figure 1.** TAS-AI hybrid workflow. The controller is implemented as **four layers, all exercised in this manuscript**. Layer 1 performs agnostic Log-GP discovery to localize signal support in accessible $(Q,E)$ space. Layer 2 performs physics-informed Hamiltonian discrimination and parameter refinement over a candidate-model family, which may be hand-specified or seeded from structure-informed proposals such as exchange-path and Goodenough-Kanamori-Anderson analysis (§6.1). Layer 3 reorders accepted probes for time-aware execution. Layer 4 is the strategic audit layer; it is fully implemented and pilot-demonstrated in §5, and is **optional at runtime** in the sense that it is enabled only when the posterior-weighted planner shows symptoms of algorithmic myopia. The dashed return arrow shows that executed measurements update the experiment state and begin the next cycle. Structure-informed Hamiltonian generation is shown only as an input to the candidate-model family in Layer 2; fully autonomous candidate-library generation from crystal structure is discussed as an outlook item in §6 rather than as part of the benchmarked workflow.

### 2.2 Sequential decision problem

We now state the autonomous experiment as a sequential decision problem.
...
Here $N$ is the total number of measurements collected so far, and the index $i$ labels the $i$th measurement in that sequence.
...
Layer 1 of the controller is an agnostic discovery policy we call **enhanced Log-GP**. It wraps the log-intensity Gaussian-process regression of Teixeira Parente *et al.* in four safeguards — coarse-grid initialization, linear-intensity variance weighting, consumed-area exclusion, and an energy-domain taper — that stabilize it for the triple-axis-spectrometer benchmark domain.
...
where $\chi^2_k$ is the weighted least-squares misfit of model $M_k$ to the current data and $p_k$ is the number of free parameters in that model.[@akaike1974] We originally tested the Bayesian Information Criterion (BIC), but in sequential settings the increasing sample count changes the BIC penalty term $p_k \ln n$ even when newly added measurements mostly reinforce already identified features rather than opening a genuinely new model-selection regime.[@schwarz1978] The issue is not that $\chi^2$ stops dominating the absolute scale, but that the relative complexity penalty drifts with $n$ in a way that is awkward for a real-time controller comparing models after each small batch. AIC keeps the complexity penalty fixed and therefore behaves more predictably as an engineering proxy for in-loop discrimination. Throughout the paper, "decisive" evidence refers to **AIC-derived evidence ratios** exceeding 100, using the Kass–Raftery Bayes-factor scale only as a heuristic reference rather than as a claim of formal marginal-likelihood evaluation.[@kass1995]

One reading convention follows from this and from §3.2, and we state it once here because it governs how every later number should be interpreted. The AIC-weight ratios are used as a guide to **model selection** — which Hamiltonian family the data favor — and the model-identity conclusions reported in this paper are robust in that sense. They are not, however, calibrated statements about the **parameter** uncertainties: as noted in §3.2, the in-loop parameter intervals are fast local Laplace surrogates whose empirical coverage is below nominal (Supplementary Note S3.4). Quoted parameter error bars should therefore be read as local ranking and routing surrogates rather than as calibrated credible intervals, even where the corresponding model selection is decisive.
...
Section 5 describes a more flexible audit layer that operates on top of these numerical mitigations in the full workflow. The two are complementary rather than redundant: the numerical mitigations are always active during normal operation, while the audit layer adds a flexible falsification channel for cases the fixed numerical rules do not anticipate. To test that channel in isolation, the controlled ablations of §5.3 deliberately switch the numerical mitigations off, so that any recovery they show is attributable to the audit channel under test rather than to the always-on safeguards.

### 3.5 Instrument model, resolution, and uncertainty

Every benchmark in §4 and §5 runs against the same digital-twin instrument model specified here.

All simulations use realistic thermal-neutron TAS parameters representative of modern triple-axis spectrometers. We assume fixed final energy $E_f=14.7$ meV, realistic horizontal and vertical collimations, pyrolytic graphite (PG; 002 reflection) monochromator/analyzer optics, and standard kinematic accessibility constraints.

Energy broadening is computed from the Cooper–Nathans (CN) resolution formalism.[@coopernathans1967a; @coopernathans1968b] At each candidate point $(Q,E)$ we evaluate the resolution matrix, extract the energy full width at half maximum, convert it to Gaussian width $\sigma_E$, and broaden the model in energy rather than performing a full four-dimensional convolution. This approximation captures the dominant effect relevant to point-by-point planning while remaining computationally lightweight. On steep dispersive branches, however, neglecting the full $Q$-$E$ coupling can modestly underestimate apparent linewidths relative to a full 4D resolution treatment.
...
This paper uses more than one spin-wave system, each matched to a specific test, so we state the roles before the equations to prevent the model changes from reading as inconsistencies. The analytic refinement, discrimination, and motion-aware studies (Figures 5–9) use a square-lattice **ferromagnet**, whose closed-form one-magnon branch is cheap enough for real-time benchmarking. The integrated closed-loop audit pilot (Figure 10, Table 3) instead uses a square-lattice **antiferromagnet** referenced to the ordering vector $\mathbf Q_{\mathrm{AF}}=(0.5,0.5,0)$, because the gapped, low-intensity regions that trigger algorithmic myopia arise naturally in the antiferromagnetic dispersion. The two controlled audit ablations of §5.3 use separate, deliberately minimal toy spectra (a fixed-$Q$ two-Lorentzian "ghost-optic" system and a bilayer ferromagnet), introduced where they are used. The choice of system in each study is deliberate and matched to what that study is meant to isolate; no result depends on reusing a single system across all of them.
...
Layer 3 of the controller — motion-aware sequencing — turns information gain into information *rate* by including motor motion costs in the acquisition denominator and, optionally, by short-horizon batch planning.
...
For the controlled scheduling diagnostic in §4.2 we compare three ordering policies against the same fixed candidate set: a **random-order** baseline that visits the candidates in shuffled order; a **nearest-neighbor** baseline that, at each step, selects the closest remaining candidate measurement in the allowed $(H,E)$ action set relative to the current instrument position, using no model-based information gain; and the full TAS-AI motion-aware ordering that maximizes information rate as defined above. The nearest-neighbor baseline is included specifically to isolate the value of *model-aware* routing from the value of *any* greedy local routing: even a purely geometric next-step rule already recovers most of the wall-clock benefit available from route ordering.
...
Figure 5 shows the controlled time-aware parameter-refinement study. Here the Hamiltonian family is assumed known and the problem is no longer blind discovery but parameter contraction under a realistic wall-clock budget. The figure should therefore be read as a refinement-stage demonstration. In this setting TAS-AI behaves as intended: it reaches the target root-mean-square (RMS) threshold after 8 measurements and about 170 s of elapsed experiment time, whereas the best competing method in the representative run (random) reaches the same threshold only after 17 measurements and about 542 s. The grid baseline finishes quickly because it exhausts a small predetermined coarse mesh; under the fixed dwell-time and motion-time accounting used here, that corresponds to only about 360 s (approximately 6 min) of elapsed time. That mesh does not sample the parameter-sensitive parts of the dispersion densely enough to contract the posterior, so the RMS parameter error does not improve substantially.
...
An autonomous loop that optimizes information gain without accounting for instrument motion can still waste beam time. Figure 7 and Table 2 isolate this effect in a **controlled scheduling study**: all methods are given the same fixed set of scientifically relevant candidate measurements, and only the ordering policy varies. The three policies — random order, nearest-neighbor, and motion-aware TAS-AI — were defined in §3.7. Motion-aware ordering reduces total run time from 88 minutes to 60 minutes for the same 50 minutes of science counting.

![Figure 7](figures/figure3_motion.png){ width=6in }

**Figure 7.** Controlled motion-aware scheduling diagnostics on a fixed candidate set. Panel (a) shows the fixed candidate measurements colored by their position in the executed schedule, so the color scale encodes traversal order rather than intensity. Panels (b)-(d) then separate the corresponding cumulative path length, elapsed time, and per-point timing summaries. `Nearest neighbor` denotes the local next-step rule that always chooses the closest remaining candidate point. The figure is intended as a scheduling diagnostic, not as a second benchmark of adaptive discovery.
...
**Figure 8.** MCTS batch planning reduces path inefficiency relative to one-step greedy ordering when motion dominates the cost budget. Panel (a) compares information rate for greedy and MCTS planning across representative batch sizes. Panel (b) shows the percent improvement from MCTS as the motor-time fraction grows for batches of 3, 5, and 8 measurements. Panel (c) shows a representative six-point trajectory comparison, where MCTS achieves a shorter path through $(H,E)$ space while maintaining comparable spectral coverage.

### 4.3 The full hybrid handoff in an integrated run

The core design principle of the paper is best seen in a single integrated run. Figure 9 shows the handoff from enhanced Log-GP discovery to physics-informed inference, with checkpoints saved at each controller-phase boundary. The automatic handoff trigger of §3.2 is implemented and is the mechanism used in normal operation; for this figure, however, we display a run with a *fixed* three-phase schedule — 13 coarse grid points, 15 enhanced Log-GP active points, then physics refinement from measurement 29 — purely so that the phase boundaries fall at known measurement indices and the control transition is visible at a glance. Run on the same seeded survey, the automatic trigger selects a comparable transition point; the fixed schedule is an exposition choice, not a substitute for an unavailable capability. The posterior remains non-decisive through the agnostic stage and sharpens to decisive support for the full model only once physics refinement begins. The posterior evolution is phase-labeled so the reader can see that the control logic is not monolithic: the system changes acquisition strategy as the task changes.
...
This subsection specifies the strict interface the LLM audit layer sees: which actions it can take, which information it can read, and what it is explicitly not allowed to do.
...
Figure 10 shows a full 90-measurement closed-loop run with the LLM audit layer active. The test system is a square-lattice antiferromagnetic (AFM) $J_1$-$J_2$-$D$ model centered on $\mathbf Q_{\mathrm{AF}}=(0.5,0.5,0)$ with four nested candidates: NN-only ($M_1$), NN+$D$ ($M_2$), NN+$J_2$ ($M_3$), and the full $J_1$+$J_2$+$D$ model ($M_4$). The synthetic data are generated from $M_4$. Starting from the enhanced Log-GP coarse grid and active warm start, the overseer alternates between agnostic remapping and physics batches while remaining inside the same constraints as the non-LLM loop. Table 3 reports the final posteriors and parameter fits from the full pilot-analysis pipeline at the end of the run, and under that final fit the full model is selected decisively. The right-hand panel is therefore retained as a compact end-state posterior summary rather than a posterior-evolution trace.

![Figure 10](figures/overseer_full90_loggpfix_20260327.png){ width=6in }

**Figure 10.** Pilot LLM-audited closed-loop run (90 measurements). The left panel shows the executed measurements over the synthetic intensity map, with enhanced Log-GP grid, enhanced Log-GP active, and physics points shown separately. The right panel shows the final model posteriors as a compact end-state summary for the pilot. The overseer operates only at the routing layer while preserving the same numerical posteriors, kinematic constraints, and safety checks as the non-LLM loop.
...
The pilot demonstration shows that the audit layer integrates into the full workflow. We now isolate the lock-in failure mode directly through two controlled ablation benchmarks of increasing physical complexity. To isolate the failure mode cleanly, these ablations deliberately disable the numerical mitigations of §3.4 (contrast-aware selection and forced diagnostic coverage): the `None` baseline is therefore pure posterior-weighted refinement with no falsification support of any kind. Any recovery seen in the other policies is then attributable to the falsification channel under test, not to the always-on safeguards used in the full workflow.
...
This parity raises the central question for the audit layer as a whole: if a three-line deterministic rule matches the LLM committee, what justifies the more complex committee at all? The multi-model stress test in Supplementary Note S5.3 is the experiment that answers it, and it is the pivot of the audit-layer argument rather than an incidental ablation. The two-model parity is expected, because there the decisive falsifier always involves the current top two candidates and is therefore visible to a top-two disagreement heuristic. The stress test is constructed so that this no longer holds: the decisive falsifier separates the current leader from a *lower-ranked* model rather than from the runner-up. This is a structural condition, not a tuning artifact — any top-two method is blind to that probe by construction, regardless of its scoring function — and it is the precise regime in which a broader falsification policy and the constrained LLM both select the critical probe while top-two Max-disagreement does not. The insight generalizes beyond neutrons: in any multi-hypothesis active-learning loop, a falsification heuristic restricted to the leading pair can be defeated whenever the discriminating evidence lies against a trailing candidate.
...
We also note that the posterior evolution in the pilot closed-loop run (Figure 10) is sensitive to the choice of prior weights: equal priors can reverse the M4/M2 ranking in the coarse checkpoint sensitivity pass of Supplementary Note S5.4. That note should not be read as a replacement for the final Table 3 analysis, which reports the end-of-run fit under the full pilot-analysis pipeline. The chemically informed prior accelerates discrimination but also shapes it, and this sensitivity should be kept in mind when interpreting the pilot results.
...
## 6. Implemented structure-informed extensions, limitations, and deployment outlook

### 6.1 Implemented structure-informed candidate generation

TAS-AI supports structure-informed Hamiltonian proposal generation from crystal structure using exchange-path analysis and Goodenough-Kanamori-Anderson heuristics, followed by data-driven discrimination among candidate models.[@goodenough1955; @kanamori1959; @anderson1950] In this workflow, structure-derived exchange pathways and orbital heuristics are used to seed a candidate-model family for the physics-informed inference layer rather than to replace downstream discrimination. Figure 13 summarizes the implemented structure-informed candidate-generation workflow used to construct that candidate list, with explicit periodic-image path enumeration and an orbital-occupancy lookup for the Goodenough-Kanamori sign heuristic.

![Figure 13](figures/exchange_path_analysis.png){ width=6in }

**Figure 13.** Implemented structure-derived candidate generation via exchange-path analysis and orbital-aware Goodenough–Kanamori heuristics. Panel A shows the periodic exchange pathways in a 2×2 supercell view, panel B summarizes the angle-based antiferromagnetic / ferromagnetic (AFM/FM) tendencies, and panel C ranks the resulting exchange channels by predicted strength.

As an additional real-material example, we apply this workflow to NiPS3 starting from the reported crystal structure and restricting the measurement geometry to a single HK0 scattering plane. In this setting, TAS-AI rejects underspecified models and identifies the J1-J2-J3(+D) Hamiltonian family from one-plane data. Broader proposal engines, including graph neural network surrogates such as CGCNN, CHGNet, and related materials models, remain compatible extensions of the same structure-to-candidate interface.[@xie2018cgcnn; @deng2023chgnet; @chen2022m3gnet]
...
The TAS-AI library — physics backends (including the analytic `SquareLatticeAFM` and `SquareFMBilayer` models used in the closed-loop pilots), the acquisition, instrument-resolution, and MCTS modules, and the core audit interface — is available at `https://github.com/usnistgov/tasai`. The manuscript source, figure-generation scripts, closed-loop drivers (`toy_closed_loop.py`, `run_audit_ablation.py`, and associated overseer wiring), benchmark summaries, prior specifications, and paper-specific provenance artifacts are available at `https://github.com/usnistgov/paper-tasai`. The version-of-record snapshots of both repositories, corresponding to the `accepted-digital-discovery` tag on each, are archived together at Zenodo: [10.5281/zenodo.20725974](https://doi.org/10.5281/zenodo.20725974). Prompt artifacts used for the pilot audit layer are described in the Supplementary Information.
The paper-facing repository also includes a pinned `environment.yml`, `REPRODUCIBILITY.md`, `REVIEWER_GUIDE.md`, and command-level scripts and provenance files for reproducing the reported figures, tables, and benchmark summaries.

## Author Contributions

W.R.: Conceptualization, Methodology, Software, Formal analysis, Investigation, Visualization, Writing - original draft, Writing - review & editing.
